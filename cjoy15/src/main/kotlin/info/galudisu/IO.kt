...
sealed class Option<out A> {
  ...
  fun <B> map(func: (A) -> B): Option<B> =
    when (this) {
      is Some -> Some(func(this.value))
      is None -> None
    }

  fun <B> flatMap(func: (A) -> Option<B>): Option<B> =
    when (this) {
...
interface Apply<F> :
  ...
  override fun <A, B> product(
    fa: Kind<F, A>,
    fb: Kind<F, B>,
  ): Kind<F, Tuple<A, B>> = ap(fb, map(fa) { a -> { b -> Tuple(a, b) } })

  fun <A, B, C> mapN(
    fa: Kind<F, A>,
    fb: Kind<F, B>,
    ff: (Tuple<A, B>) -> C,
  ): Kind<F, C> = map(fa = product(fa, fb), func = ff)
}
...
interface Monad<F> :
  ...
  override fun <A, B> map(
    fa: Kind<F, A>,
    func: (A) -> B,
  ): Kind<F, B> = flatMap(fa) { pure(func(it)) }
...
sealed class Free<F, out A> {
  ...
  @Suppress("UNCHECKED_CAST")
  tailrec fun <F, A> Functor<F>.resume(free: Free<F, A>): Either<Kind<F, Free<F, A>>, A> =
    when (free) {
  ...
  internal class Pure<F, out A>(
    val a: A,
  ) : Free<F, A>()

  internal class Suspend<F, out A>(
    val fa: Kind<F, A>,
  ) : Free<F, A>()

  internal class FlatMapped<G, In, out Out>(
    val self: Free<G, In>,
    val func: (In) -> Free<G, Out>,
  ) : Free<G, Out>()
...
sealed class IO<out A> {
  ...
  internal class Pure<out A>(
    val value: A,
  ) : IO<A>()

  internal class Effect<out A>(
    val resume: () -> A,
  ) : IO<A>()

  internal class FlatMapped<A, out B>(
    val sub: IO<A>,
    val f: (A) -> IO<B>,
  ) : IO<A>()
  ...
  internal class Recover<out A>(
    self: IO<A>,
    handler: (Throwable) -> IO<A>,
  ) : IO<A>()

  internal class Async<out A>(
