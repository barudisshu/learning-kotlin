...
sealed class List<out A> {
  ...
  fun zipWithPosition(): List<Pair<A, Int>> = zipWith(this, range(0, this.length)) { a -> { i: Int -> Pair(a, i) } }
  ...
  fun exists(p: (A) -> Boolean): Boolean = foldLeft(false, true) { x -> { y: A -> x || p(y) } }.first
...
//    }
  ...
  fun <B> foldRight(
    identity: B,
    f: (A) -> (B) -> B,
  ): B = foldRight(this, identity, f)

  fun <B> foldLeft(
    identity: B,
    f: (B) -> (A) -> B,
  ): B = foldLeft(identity, this, f)

  fun length(): Int = length

  fun <B> foldRightViaFoldLeft(
    identity: B,
    f: (A) -> (B) -> B,
  ): B = this.reverse().foldLeft(identity) { x -> { y -> f(y)(x) } }

  fun <B> coFoldRight(
    identity: B,
    f: (A) -> (B) -> B,
  ): B = coFoldRight(identity, this.reverse(), identity, f)
  ...
  companion object {
    ...
    fun <A> concat(
      list1: List<A>,
      list2: List<A>,
    ): List<A> = list1.reverse().foldLeft(list2) { x -> x::cons }

    fun <A> concatViaFoldRight(
      list1: List<A>,
      list2: List<A>,
    ): List<A> = foldRight(list1, list2) { x -> { y -> Cons(x, y) } }
    ...
    fun fromSeparated(
      string: String,
      separator: String,
    ): List<String> = List(*string.split(separator).toTypedArray())
  }
}
...
tailrec fun <A> lastSafe(list: List<A>): Result<A> =
  when (list) {
    List.Nil -> Result()
    is List.Cons<A> ->
      when (list.tail) {
