...
fun <A, B> hList(f: (A) -> B): (A) -> Option<B> =
  {
    try {
      Option(it).map(f)
    } catch (e: Exception) {
...
val upperOption: (Option<String>) -> Option<String> = lift { it.uppercase(Locale.getDefault()) }

val upperOption_: (Option<String>) -> Option<String> = lift(String::uppercase)
...
fun <A> sequence(list: List<Option<A>>): Option<List<A>> = traverseOption(list) { x -> x }
