...
sealed class Heap<out A> {
  ...
  fun <B> foldLeft(
    identity: B,
    f: (B) -> (A) -> B,
  ): B = unfold(this, { it.pop() }, identity, f)

  private fun <A, S, B> unfold(
    z: S,
  ...
  companion object {
    ...
    ): Heap<A> = H(1, Empty(Result(comparator)), element, Empty(Result(comparator)), Result(comparator))

    protected fun <A> merge(
      head: A,
      first: Heap<A>,
      second: Heap<A>,
    ): Heap<A> =
